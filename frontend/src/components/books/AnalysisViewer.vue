...
const formattedAnalysis = computed(() => {
  ...
  let html = analysis.value
    ...
    .replace(/^# (.+)$/gm, '<h1 class="text-2xl font-bold mt-8 mb-4 text-gray-900 border-b pb-2">$1</h1>')
    // Bold and italic
    .replace(/\*\*\*(.+?)\*\*\*/g, '<strong><em>$1</em></strong>')
    .replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>')
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    // Code blocks
    .replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="bg-gray-100 p-4 rounded my-4 overflow-x-auto"><code>$2</code></pre>')
  ...
  <Teleport to="body">
    <Transition name="slide">
      ...
      >
        ...
        <!-- Panel -->
        <div class="relative ml-auto w-full max-w-3xl bg-white shadow-xl h-full overflow-hidden flex flex-col">
          <!-- Header -->
